...
function hostnameFromNode(node) {
    ...
    if ( li === null ) { return; }
    const span = qs$(li, '.hostname[data-before]');
    if ( span === null ) { return; }
    return span.textContent || undefined;
}

function selectorFromNode(node) {
    const li = node.closest('li.selector');
    if ( li === null ) { return; }
    const span = qs$(li, '.selector[data-before]');
    if ( span === null ) { return; }
...
async function renderCustomFilters() {
    ...
    for ( const hostname of hostnames ) {
        ...
        label.dataset.before = hostname;
        dom.text(label, hostname);
...
async function onHostnameChanged(target, before, after) {
    if ( isValidHostname(after) === false ) {
...
async function onSelectorChanged(target, before, after) {
    ...
    target.dataset.before = result.raw;
    await sendMessage({ what: 'addCustomFilters',
        hostname,
...
function onTextChanged(target) {
    const before = target.dataset.before;
...
function endEdit(ev) {
    const { target } = ev;
    if ( target.textContent !== target.dataset.before ) {
